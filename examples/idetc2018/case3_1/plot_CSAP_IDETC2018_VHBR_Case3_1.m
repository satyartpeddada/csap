%--------------------------------------------------------------------------
% plot_CSAP_IDETC2018_VHBR_Case3_1.m
% 
%--------------------------------------------------------------------------
%
%--------------------------------------------------------------------------
...
xmax = min(max(t(:)),750); % x axis maximum
ymin = min(X(:)-2); % x axis minimum
ymax = max([X(:);p.xmax(1)]+2); % x axis maximum
...
if xmax == 750
    ...
    mytxt = num2str(round(xmax,0));
end

% text(xmax,ymin,num2str(round(xmax)),'interpreter','latex');
text(xmax+0.2,ymin,{'','$t_{\mathrm{end}}$',['$',mytxt,'$'],'',''},...
...
% add the tree
ax1 = axes('Position',[0.6 0.24 0.3 0.35]);
ax1.Visible='off';
